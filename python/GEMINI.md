...
This directory contains a collection of standalone Python utility scripts and
tools used for various system administration and personal tasks. These scripts
are designed to be run directly from the command line.
...
- **Language:** Python 3 (specifically compatible with 3.14 or later).
- **Style guide:**
  - Always adhere to the official Google Python Style Guide:
    <https://github.com/google/styleguide/blob/gh-pages/pyguide.md>
  - Prioritize these guidelines for code formatting, naming conventions, naming
    restrictions, and docstring formats.
...
- **Data Structures:** `dataclasses` are preferred over dictionaries for
  structured data. `typing.TypedDict` is required when processing YAML, TOML, or
  JSON.
...
- **Linting:** Code must be linted with `basedpyright`.
- **Dependencies:** Standard library is heavily utilized, but some scripts
  import external modules (e.g., `pyfakefs` in tests).
- **Testing:**
